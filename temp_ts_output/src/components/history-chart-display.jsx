...
import { fetchRateHistory } from "@/lib/currency-api"; // Updated import
import { LineChart, Line, XAxis, YAxis, Tooltip, ResponsiveContainer, ReferenceArea } from 'recharts';
import { Badge } from '@/components/ui/badge';
import { BANDS, classifyRateToBand, getStaticBandColorConfig, } from "@/lib/bands";
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select";
const BandLabel = ({ viewBox, value, textColorCssVar }) => {
    ...
    const dx = 10;
    const dy = height / 2;
    return (<text x={x + dx} y={y + dy} fill={textColorCssVar} // Use the CSS variable directly
     fontSize={11} fontWeight="bold" textAnchor="start" dominantBaseline="middle" className="pointer-events-none">
      {value}
    </text>);
};
const HistoryChartDisplay = ({ alertPrefs, fromCurrency, toCurrency, }) => {
    ...
    const bandUIDefinitions = useMemo(() => {
        return BANDS.map(b => {
            var _a, _b;
            ...
            return {
                ...
                y1: (_a = b.minRate) !== null && _a !== void 0 ? _a : undefined,
                y2: (_b = b.maxRate) !== null && _b !== void 0 ? _b : undefined,
    ...
    const yAxisDomain = useMemo(() => {
        let minDataRate = undefined;
        let maxDataRate = undefined;
        ...
        if (fromCurrency !== 'USD' || toCurrency !== 'THB') {
            if (minDataRate === undefined || maxDataRate === undefined) {
                ...
                return [typicalRate * 0.8, typicalRate * 1.2];
            }
            const range = maxDataRate - minDataRate;
            const padding = range === 0 ? 0.1 * Math.abs(minDataRate) || 0.1 : range * 0.10;
            return [parseFloat((minDataRate - padding).toFixed(4)), parseFloat((maxDataRate + padding).toFixed(4))];
        }
        // Logic for USD/THB with bands
        const activeBandNumericBoundaries = [];
        bandUIDefinitions.forEach(bandDef => {
            if (alertPrefs[bandDef.level]) {
                if (bandDef.y1 !== undefined)
                    activeBandNumericBoundaries.push(bandDef.y1);
                if (bandDef.y2 !== undefined)
                    activeBandNumericBoundaries.push(bandDef.y2);
            }
        });
        let overallMin = minDataRate !== null && minDataRate !== void 0 ? minDataRate : 30; // Default for USD/THB if no data
        let overallMax = maxDataRate !== null && maxDataRate !== void 0 ? maxDataRate : 36; // Default for USD/THB if no data
        ...
        return [parseFloat((overallMin - padding).toFixed(2)), parseFloat((overallMax + padding).toFixed(2))];
    ...
    const CustomTooltip = ({ active, payload, label }) => {
        if (active && payload && payload.length) {
            ...
            return (<div className="bg-background/90 backdrop-blur-sm p-3 border border-border rounded-lg shadow-xl">
          ...
          {bandForTooltip && (<div className="mt-1">
    ...
    const chartTitle = useMemo(() => {
        ...
        let periodDesc = "";
        if (periodInDays === 30)
            periodDesc = "30-Day Trend";
        else if (periodInDays === 90)
            periodDesc = "90-Day Trend";
        else if (periodInDays === 180)
            periodDesc = "180-Day Trend";
        else if (periodInDays === 365)
            periodDesc = "1-Year Trend";
        else if (periodInDays === (5 * 365))
            periodDesc = "5-Year Trend";
            ...
            if (startYear.toString() === defaultStartYear && chartData.length === 0)
                return `Historical Trend (${pair})`;
    ...
    return (<Card className="overflow-hidden shadow-lg rounded-xl">
      ...
      <CardContent className="pt-6 pb-2 bg-background">
        {(isLoading && (!chartData || chartData.length === 0)) ? (<div className="h-[350px] flex items-center justify-center">
            <Loader2 className="h-8 w-8 animate-spin text-primary"/>
          </div>) : (!chartData || chartData.length === 0) && !isLoading ? (<div className="h-[350px] flex items-center justify-center text-muted-foreground text-center px-4">
            ...
            <LineChart data={chartData} margin={{ top: 5, right: 30, left: 25, bottom: 5 }}>
              <XAxis dataKey="date" tickFormatter={(value) => {
                ...
                if (periodInDays <= 365 * 2)
                    return date.toLocaleDateString('en-US', { month: 'short', year: '2-digit' });
                return date.toLocaleDateString('en-US', { year: 'numeric' });
            }} tick={{ fontSize: 11, fill: 'hsl(var(--muted-foreground))' }} tickLine={{ stroke: 'hsl(var(--border))' }} axisLine={{ stroke: 'hsl(var(--border))' }} interval="preserveStartEnd" minTickGap={periodInDays > 365 ? 60 : 40} // Wider gap for longer periods
        />
              <YAxis domain={yAxisDomain} tickFormatter={(value) => typeof value === 'number' ? value.toFixed(toCurrency === 'JPY' ? 3 : 4) : ''} // More precision for JPY like pairs
         tick={{ fontSize: 11, fill: 'hsl(var(--muted-foreground))' }} tickLine={{ stroke: 'hsl(var(--border))' }} axisLine={{ strokeWidth: 1, stroke: 'hsl(var(--border))' }} allowDataOverflow={true} width={80} // Increased width for potentially longer rate numbers
         label={{
              ...
              {fromCurrency === 'USD' && toCurrency === 'THB' && bandUIDefinitions.map((bandDef) => {
                var _a, _b;
                if (alertPrefs[bandDef.level]) {
                    let y1Actual = (_a = bandDef.y1) !== null && _a !== void 0 ? _a : yAxisDomain[0];
                    let y2Actual = (_b = bandDef.y2) !== null && _b !== void 0 ? _b : yAxisDomain[1];
                    ...
                    if (finalY1 >= finalY2 && !(bandDef.level === "RICH" && finalY1 === yAxisDomain[1])) {
                        if (bandDef.level === "RICH" && finalY1 > yAxisDomain[1])
                            return null;
                    }
                    const bandColorConfig = getStaticBandColorConfig(bandDef.level);
                    return (<ReferenceArea key={bandDef.level} y1={finalY1} y2={finalY2} fill={bandColorConfig.chartSettings.fillVar} stroke={bandColorConfig.chartSettings.strokeVar} strokeWidth={0.5} fillOpacity={1} strokeOpacity={1} ifOverflow="visible" label={<BandLabel value={bandDef.displayName} textColorCssVar={bandColorConfig.chartSettings.labelTextColorVar}/>}/>);
                }
                return null;
            })}

              <Line type="monotone" dataKey="rate" stroke="hsl(var(--primary))" strokeWidth={2} dot={{ r: 0 }} activeDot={{ r: 5, stroke: 'hsl(var(--background))', strokeWidth: 2, fill: 'hsl(var(--primary))' }} name={`${toCurrency}/${fromCurrency} Rate`}/>
            </LineChart>
          </ResponsiveContainer>)}
      </CardContent>
    </Card>);
