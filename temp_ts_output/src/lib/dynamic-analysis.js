...
import { fetchRateHistory } from './currency-api';
export async function generatePairAnalysis(fromCurrency, toCurrency, days = 365 * 5) {
...
function calculateDistributionStatistics(rates) {
...
function generateTrendSummary(rates, numSegments = 3 // Aim for 3-4 segments
) {
    ...
    const trendPeriods = [];
    let previousSegmentAvgRate = null;
    for (let i = 0; i < adjustedNumSegments; i++) {
        ...
        if (segmentRates.length === 0)
            continue; // Should not happen with rates.length >= 20
...
function generateThresholdBands(stats, _rates) {
