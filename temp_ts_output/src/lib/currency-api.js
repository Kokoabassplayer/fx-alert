...
export async function fetchCurrentRate(from, to) {
    try {
        // Add a cache-busting query parameter
        const timestamp = Date.now();
        const response = await fetch(`${API_BASE_URL}/latest?from=${from}&to=${to}&t=${timestamp}`, { cache: 'no-store' });
        if (!response.ok) {
            console.error("Failed to fetch current rate (HTTP status):", response.status, await response.text().catch(() => "Could not read response text"));
            return null;
        }
        let data;
        try {
            ...
            console.error("Failed to parse JSON response for current rate:", jsonError, await response.text().catch(() => "Could not read response text (after JSON parse failure)"));
            return null;
        }
...
export async function fetchRateHistory(from, to, days = 90) {
    ...
    try {
        ...
        if (!response.ok) {
            console.error(`Failed to fetch rate history for ${from}-${to} (HTTP status):`, response.status, await response.text().catch(() => "Could not read response text"));
            return [];
        }
        let data;
        try {
            ...
            console.error(`Failed to parse JSON response for ${from}-${to} rate history:`, jsonError, await response.text().catch(() => "Could not read response text (after JSON parse failure)"));
            return [];
        }
        if (typeof data !== 'object' || data === null) {
...
export function aggregateMonthlyRates(dailyRates) {
    ...
    const aggregatedRates = Object.entries(monthlyData)
        .map(([yearMonth, data]) => ({
        yearMonth,
...
export async function fetchAvailableCurrencies() {
    try {
        ...
        if (!response.ok) {
            console.error("Failed to fetch available currencies (HTTP status):", response.status, await response.text().catch(() => "Could not read response text"));
            return null;
        }
        let data;
        try {
            ...
            console.error("Failed to parse JSON response for available currencies:", jsonError, await response.text().catch(() => "Could not read response text (after JSON parse failure)"));
            return null;
        }
        if (typeof data !== 'object' || data === null) {
