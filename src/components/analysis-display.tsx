...
const actionableThresholdsData = threshold_bands.map(band => {
  ...
  if (band.range.max === null && band.range.min !== null) {
    ...
    // Adjusted to use the exact values from JSON for DEEP, OPPORTUNE, NEUTRAL
     ...
     else rangeDisplay = `${band.range.min.toFixed(1)} – ${band.range.max.toFixed(1)}`;
  } else {
    rangeDisplay = "N/A";
  }
    ...
    <div className="space-y-6">
      ...
      <Card className="overflow-hidden shadow-lg rounded-xl">
       ...
       <p className="text-xs text-muted-foreground text-center mt-4">
            Analysis data generated by GPT-3. Application developed by Nuttapong Buttprom using Firebase Studio.
       </p>
    </div>
  );
};
